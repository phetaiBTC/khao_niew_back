    ...
    return this.transactionManager.runInTransaction(
      ...
      async (manager) => {
        try {
          ...
          if (detailsScan) {
  // If record exists for this company, update the amount and add the new CheckIn ID
  detailsScan.amount += 1;
  detailsScan.check_in_ids.push(savedCheckIn.id);
