...
export class CheckInService {
  ...
  async create(createCheckInDto: CreateCheckInDto) {
    await this.bookingDetailRepository.update(createCheckInDto.booking_details, { status: DetailsStatus.CHECKED_IN });
      ...
      async (manager) => {
        try {
          ...
          await manager
            ...
            .set({ status: DetailsStatus.CHECKED_IN })
            .where('id = :id', { id: bookingDetail.id })
            .execute();
