...
export class CheckInService {
  ...
  async create(createCheckInDto: CreateCheckInDto) {
    const bookingDetails = await this.bookingDetailRepository.findOneBy({ id: createCheckInDto.booking_details });
    bookingDetails.status = DetailsStatus.CHECKED_IN;
    await this.bookingDetailRepository.save(bookingDetails);
    const   this.transactionManager.runInTransaction(
      this.dataSource,
      async (manager) => {
        try {
          ...
          const savedCheckIn = await manager.save(CheckIn, checkIn);
            ...
            //update status
            bookingDetail.status = DetailsStatus.CHECKED_IN;
            await manager.save(BookingDetail, bookingDetail);
            console.log('Saved BookingDetail:', bookingDetail.status);
            const updated = await manager.findOne(BookingDetail, { where: { id: bookingDetail.id } });
            if (updated) {
              console.log('DB BookingDetail after save:', updated.status);
            } else {
              console.log('BookingDetail not found');
            }
