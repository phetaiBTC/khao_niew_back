...
export class CheckInService {
    ...
    return this.transactionManager.runInTransaction(
      ...
      async (manager) => {
        try {
         ...
         if (detailsScan) {
  // ถ้ามีแล้ว: อัพเดตจำนวน และผูก CheckIn เข้ากับ detailsScan
  detailsScan.amount += 1;
  savedCheckIn.details_scans = [detailsScan]; // set relation
  ...
  savedCheckIn.details_scan = [detailsScan]; // set relation
  await manager.save(CheckIn, savedCheckIn);
}
