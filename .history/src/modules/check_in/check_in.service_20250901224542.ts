    ...
    return this.transactionManager.runInTransaction(
      ...
      async (manager) => {
        try {
          ...
          const savedBookingDetail =  manager.create(BookingDetail, bookingDetail);
          await manager.save(BookingDetail, savedBookingDetail);
          console.log('Saved BookingDetail:', savedBookingDetail.status);
          const relations = ['booking'];
          const allDetails = await this.bookingDetailRepository.find({ relations: ['booking', 'otherRelation', 'anotherRelation'] });
          // Get the company ID
