    ...
    try {
      const result = await this.transactionManagerService.runInTransaction(
        ...
        async (manager) => {
         ...
         const concert = await manager.getRepository(Concert).findOne({
            where: {
                id: createBookingDto.concertId
            }
         })
         ...
         await manager.getRepository(Concert).update(createBookingDto.concertId, {
            limit: concert.limit - createBookingDto.ticket_quantity
