  ...
  async validateQrCode(ticketCode: string) {
    try {
      ...
      try {
        ...
        const qrCodeData = JSON.parse(qrDataString);
        ...
        console.log('QR code hash:', qrCodeData.hash);

        if (calculatedHash !== qrCodeData.hash) {
          throw new NotFoundException('Invalid QR code hash - hash mismatch');
        }
