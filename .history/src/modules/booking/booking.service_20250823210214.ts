...
export class BookingService {
  ...
  constructor(
     @InjectDataSource() private readonly dataSource: DataSource,
        ...
        @InjectRepository(Concert) private readonly bookingRepository: Repository<Concert>,
  ) {}
  async create(createBookingDto: CreateBookingDto) {
  try {
     // Check if the concert exists before attempting to insert a new booking
    const concert = await this.concertRepositor.findOne(createBookingDto.concert);
    if (!concert) {
      throw new Error(`Concert with id ${createBookingDto.concert} does not exist`);
    }
