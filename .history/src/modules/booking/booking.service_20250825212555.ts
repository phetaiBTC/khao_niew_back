...
export class BookingService {
  ...
  try {
    ...
    const check_concert_in_bookings = await this.bookingRepository.find({ where: { concert: { id: createBookingDto.concert } } });
    const total_tickets_booked = check_concert_in_bookings.reduce((sum, booking) => sum + booking.ticket_quantity, 0);
    if (total_tickets_booked + createBookingDto.ticket_quantity > concert.total_tickets) {
      throw new NotFoundException(`Not enough tickets available for concert with id ${createBookingDto.concert}`);
    }
