...
export class BookingService {
  ...
  try {
    const user = await this.dataSource.getRepository(User).findOne({where:{id:createBookingDto.user}});
    if(!user){
      throw new NotFoundException(`User with id ${createBookingDto.user} does not exist`);
    }
    // 1.
    ...
    const result = await this.transactionManagerService.runInTransaction(
      ...
      async (manager) => {
        ...
        const payment =  manager.create(Payment,{
         ...
         const calualte = createBookingDto.unit_price * createBookingDto.ticket_quantity;
        const booking =  manager.create(Booking,{
          ticket_quantity: createBookingDto.ticket_quantity,
          unit_price: createBookingDto.unit_price,
          total_amount: calualte ,          
